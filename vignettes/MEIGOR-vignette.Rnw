...
% 	%ALEX, autthors list
% 	
 	\author[1]{Jose A. Egea\thanks{josea.egea@gmail.com}}
 	\author[2]{David Henriques}
 	\author[3]{Thomas Cokelaer}
 	\author[4]{Alejandro F. Villaverde}
 	\author[5]{Julio R. Banga \thanks{j.r.banga@csic.es}}
 	\author[6]{Julio Saez-Rodriguez \thanks{pub.saez@uni-heidelberg.de}}
 
 	\affil[1]{Centro de Edafolog\'ia y Biolog\'ia Aplicada del Segura, Murcia, Spain}
 	\affil[2]{Instituto de Investigaciones Marinas-CSIC, Vigo, Spain}
 	\affil[3]{Institute Pasteur, Paris, France}
 	\affil[4]{University of Vigo, Vigo, Spain}
 	\affil[5]{Misi\'on Biol\'oxica de Galicia, Pontevedra, Spain}
 	\affil[6]{Universit\"at Heidelberg, Heidelberg, Germany}
...
\section{Introduction}
	 ...
	 and other areas. It consists of two main metaheuristics: the enhanced scatter search method, {\it eSSR} \cite{Egea_et_al:09,Egea_et_al:10} for continuous and mixed-integer problems, and the variable neighbourhood search metaheuristic \cite{Hansen_Mladenovic_Moreno-Perez:10}, for integer problems.
...
%DELETE THIS PARAGRAPH WHEN THE NL2SOL ISSUE IS SOLVED
  	
  	  	where $x$ is the vector of decision variables, and $x_L$ and $x_U$ its respective bounds. $p_1,\ldots,p_n$ are optional extra input parameters to be passed to the objective function (see example in section \ref{sec:use_param1}). $c_{eq}$ is a set of equality constraints. $c(x)$ is a set of inequality constraints with lower and upper bounds, $c_L$ and $c_U$. Finally, $f(x,p_1,p_2,...,p_n)$ is the objective function to be minimized.
...
\subsubsection{User options}
		 ...
		 \begin{itemize}
			...
			\item \textbf{log\_var}: Indexes of the variables which will be analyzed using a logarithmic distribution instead of an uniform one\footnote[3]{Useful when the bounds of a decision variables have different orders of magnitude and they are both positive.} (default: numeric(0)).
			%RECOVER THIS PARAGRAPH WHEN THE NL2SOL ISSUE IS SOLVED
		...
		\item \textbf{save\_results}: Saves final results in \textbf{eSSR\_report.Rdata}. 0: deactivated; 1: activated (default: 1).
...
\noindent The solver is called in \texttt{main\_ex3.R}. The global optimum for this problem is located in $x^*=[0.77152, 0.516994, 0.204189, 0.388811, 3.0355, 5.0973]$ with $f(x^*)= -0.388811$.
...
as local solver is presented in the script \texttt{main\_multistart\_ex3.R}. The number of initial points chosen is 10.
...
The field \textit{n+1} should be named hosts. opts\$hosts should be a vector containing as many elements
	as threads you want to launch in parallel. In case you are using the 'SOCKS' mechanism from snowfall each element of this vector should be a string with the address of the machines where you want to launch the optimization. For instance, opts\$hosts('node1','node1','node2','node2','node5','localhost','127.0.0.1') will use 7 threads for each iteration;
 	...
 	\begin{itemize}
 		\item \textbf{ce\_maxeval}: The maximum number of evaluations. By default this is set to infinity leaving the stopping criterion to the number of iterations.
...
visualize the obtained network (see figure ~\ref{fig:CNORode_plot_model2}).
