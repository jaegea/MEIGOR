#' @include calculate_prior.R
#' @include calculate_likelihood.R
#' 
#' @title Calculate posterior from prior and likelihood
#' 
#' @description Calculates the prior and likelihood and adds them together (with the
#'    likelihood scaled by the thermodynamic integration temperature, if
#'    being used) to generate the posterior. Returns a tuple of the posterior,
#'    prior and likelihood as all three always tracked together by the algorithm.
#' 
#' 
...
#'    
#' 
#' @return Return the prior, likelihood and posterior for a position.   
#' @return posterior: Value of the posterior calculated at 'position'.
#' @return prior: Value of the prior calculated at 'position'.
#' @return likelihood: Value of the likelihood calculated at 'position'. 

calculate_posterior = function(output,options, position=NULL){
  prior = calculate_prior(output,options,position)
  likelihood = calculate_likelihood(output,options,position)
  posterior = prior + likelihood * options$thermo_temp
  return(c(posterior, prior, likelihood))
}
