#' @title Calculate the prior at a certain  position.
#' 
#' 
...
#'    
#'    
#' @return Return the prior for a position in parameter space.
#'    If 'options$prior_fn' is NULL, return 0 (i.e. a flat prior).

calculate_prior <- function(output, options, position=NULL){
  if (is.null(position)){
    position = output$position
  }  
  if (!is.null(options$prior_fn)){
    return(options$prior_fn(position))
  } else {
