#' @title A helper function that returns the thinned, 
...
#' 
#' @description Use this method instead of ``prune`` if you want to get the 
...
#'    
#'    
#' @return Returns a tuple of a array objects. The first element in the tuple contains
#'    the array of accepted positions, burned and thined as required; the second
#'    element contains a list of integers which indicate the indeces associated
#'    with each of the steps returned.
#'    

get_mixed_accepts = function(output, options, burn, thin=1){
  mixed_steps = c(burn:options$nsteps)
  mixed_positions = output$positions[burn:options$nsteps,]

  mixed_accepts = mixed_positions[output$accepts[burn:options$nsteps],]
  mixed_accept_steps = mixed_steps[output$accepts[burn:options$nsteps]]
  
  thinned_accepts = mixed_accepts[seq(from=1, to=dim(mixed_accepts)[1], by=thin),]
  thinned_accept_steps = mixed_accept_steps[seq(from=1, to=dim(mixed_accepts)[1], by=thin)]
  
  return(list("thinned_accepts"=thinned_accepts, "thinned_accept_steps" = thinned_accept_steps))
}
